...
class AngieModelJoomlaSetup extends AngieModelBaseSetup
{
	...
	public function applySettings()
	{
		...
		// Forcibly set Force SSL to 0 if requested so
		if ($this->getState('disableForceSSL', 0))
		{
			$this->configModel->set('force_ssl', 0);
		}
	...
	private function applyServerconfigchanges()
	{
		...
		if ($this->input->get('removeaddhandler'))
		{
	...
	/**
	 * Removes the lines regarding AddHandler
	 */
	private function removeAddHandler()
	{
		...
		foreach ($files as $file)
		{
			$target_file = APATH_ROOT.'/'.$file;

			// File is not there? Skip it
			...
			// No Handler? Skip it
			if (stripos($contents, 'addhandler') === false)
			{
				continue;
			}
			...
			{
				// Skip lines containing AddHandler rules
				if (stripos($line, 'addhandler') !== false)
				{
					continue;
				}
			...
			// If we can't write the contents back to the file, immediately stop. Most likely there are some permission
			// issues, there's no point in continuing
			if (!file_put_contents($target_file, $new_data))
			{
				return false;
			}
		}
	...
	public function hasAddHandler()
	{
		...
		foreach ($files as $file)
		{
			...
			if (strpos($contents, 'AddHandler') !== false)
			{
				return true;
			}
		}
